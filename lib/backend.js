...
exports.createServer = (dynamodb, docClient) => {
  ...
  const listAllTables = (lastEvaluatedTableName, tableNames) => {
    return listTables({ ExclusiveStartTableName: lastEvaluatedTableName }).then(
      data => {
  ...
  app.get(
    ...
    asyncMiddleware((req, res) => {
      return listAllTables(null, []).then(data => {
  ...
  app.delete(
    ...
    asyncMiddleware((req, res) => {
      return purgeTable(req.params.TableName, dynamodb).then(() => {
        res.status(200).end();
      });
    })
  );
